...
static eCallback_fn onRead;
static eCallback_fn onWrite;
	...
	switch (event) {
	  ...
	  // gl_profile_tab[nowIndex].service_id.id.uuid.uuid.uuid16 = service_char_uuid_t[nowIndex].service_uuid;
			...
			if (onRead) {
				for (int i=0;i<gl_profile_tab[nowIndex].service->_char_count;i++) {
					if (param->read.handle == gl_profile_tab[nowIndex].service->_handle_id[i]) {
						// onRead(gl_profile_tab[nowIndex].service_id.id.uuid.uuid.uuid16, gl_profile_tab[nowIndex].service->_arr_char_uuid[i]);
			...
			if (onWrite) {
				...
				for (int i=0;i<gl_profile_tab[nowIndex].service->_char_count;i++) {
					if (param->write.handle == gl_profile_tab[nowIndex].service->_handle_id[i]) {
						// onWrite(gl_profile_tab[nowIndex].service_id.id.uuid.uuid.uuid16, gl_profile_tab[nowIndex].service->_arr_char_uuid[i]);
		...
		case ESP_GATTS_CREATE_EVT:
			...
			// gl_profile_tab[nowIndex].char_uuid.len = ESP_UUID_LEN_16;
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
  ...
  do {
    for (int idx = 0; idx < nowCount_profile; idx++) {
